...
from Utils.ObjectTrackers.DeepSort.Helper.deep_sort.deep.reid.torchreid.utils import read_image, download_url, mkdir_if_missing
...
class ImageDataset(Dataset):
    ...
    It will return ``orig_img``, ``pid``, ``camid`` and ``img_path``
    where ``orig_img`` has shape (channel, height, width). As a result,
...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for index in indices:
            ...
            img = img.unsqueeze(0) # orig_img must be torch.Tensor
            imgs.append(img)
        imgs = torch.cat(imgs, dim=0)

        item = {'orig_img': imgs, 'pid': pid, 'camid': camid, 'dsetid': dsetid}
